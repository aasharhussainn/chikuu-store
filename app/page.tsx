  ...
  return (
    <main className="min-h-screen bg-white text-black font-sans relative">
      <nav className="fixed top-0 left-0 w-full flex justify-between items-center py-2 px-4 md:py-3 md:px-8 border-b border-gray-100 bg-white/90 backdrop-blur-md z-[150]">
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} onClick={() => { setActiveCategory(null); setIsAboutOpen(false); setIsTypesOpen(false); }} className="cursor-pointer">
          <img src="/logo.png" alt="Chikuu Logo" className="h-12 md:h-20 w-auto object-contain" />
        </motion.div>
      ...
      <AnimatePresence>
        {isAboutOpen && (
          <motion.div initial={{ opacity: 0, x: "-100%" }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: "-100%" }} transition={{ type: "spring", damping: 25, stiffness: 200 }} className="fixed inset-0 bg-white z-[140] overflow-y-auto">
            <div className="fixed bottom-8 right-8 z-[150]">
              <motion.button onClick={() => setIsAboutOpen(false)} whileHover={{ scale: 1.1 }} whileTap={{ scale: 0.9 }} className="bg-black text-white px-6 py-3 rounded-full font-bold text-sm shadow-2xl border border-[#C5A059] flex items-center gap-2">
      ...
      <AnimatePresence>
        {activeCategory && (
          <motion.div initial={{ opacity: 0, x: "100%" }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: "100%" }} transition={{ type: "spring", damping: 25, stiffness: 200 }} className="fixed inset-0 bg-white z-[140] overflow-y-auto">
            <div className="fixed bottom-8 right-8 z-[150]">
              <motion.button onClick={() => setActiveCategory(null)} whileHover={{ scale: 1.1 }} whileTap={{ scale: 0.9 }} className="bg-black text-white px-6 py-3 rounded-full font-bold text-sm shadow-2xl border border-[#C5A059] flex items-center gap-2">
                <span className="w-2 h-2 bg-[#C5A059] rounded-full"></span>← BACK
              </motion.button>
            </div>
      ...
      {!activeCategory && !isAboutOpen && (
        <>
          <section className="flex flex-col items-center justify-center text-center mt-12 md:mt-24 px-6 mb-16 md:mb-32">
            <motion.h2 initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} whileHover={{ scale: 1.08, color: "#C5A059" }} transition={{ duration: 0.8 }} className="text-4xl md:text-7xl font-extrabold tracking-tight mb-4 md:mb-6 uppercase cursor-default">
            ...
            <motion.button onClick={() => setIsTypesOpen(true)} whileHover={{ scale: 1.05, backgroundColor: "#000000" }} whileTap={{ scale: 0.95 }} className="w-full md:w-auto bg-[#C5A059] text-white px-12 py-4 rounded-full text-sm font-bold tracking-widest uppercase shadow-lg transition-colors duration-300">
              View Categories
            </motion.button>
          </section>
      ...
      <div className="fixed bottom-20 md:bottom-24 right-4 md:right-8 z-[100]">
        <motion.a href={`https://wa.me/${whatsappNumber}?text=${encodeURIComponent(whatsappMessage)}`} target="_blank" rel="noopener noreferrer" whileHover={{ scale: 1.1 }} whileTap={{ scale: 0.9 }} className="bg-[#25D366] text-white p-3 md:p-4 rounded-full shadow-2xl flex items-center justify-center border-2 border-white cursor-pointer">
          ...
          <>
            ...
            <motion.div initial={{ x: "100%" }} animate={{ x: 0 }} exit={{ x: "100%" }} transition={{ type: "spring", damping: 25, stiffness: 200 }} className="fixed right-0 top-0 h-full w-full md:w-[450px] bg-white z-[210] p-6 md:p-10 shadow-2xl overflow-y-auto">
                ...
                <div className="mb-6">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {selectedProduct.sizes.map((size) => (
                      <button key={size} onClick={() => setChosenSize(size)} className={`border px-3 py-1.5 md:px-4 md:py-2 rounded-lg text-[10px] md:text-sm font-bold transition-all uppercase ${chosenSize === size ? "bg-black text-white border-black" : "border-gray-200 text-black hover:border-[#C5A059]"}`}>{size}</button>
                    ))}
                  </div>
                </div>
              )}
              ...
              {selectedProduct.inStock === false ? (
                <button disabled className="w-full bg-gray-200 text-gray-400 py-4 md:py-5 rounded-full font-bold tracking-widest uppercase cursor-not-allowed border border-gray-300 text-xs md:text-sm">OUT OF STOCK</button>
              ) : (
                <button onClick={() => addToCart(selectedProduct)} className="w-full bg-black text-white py-4 md:py-5 rounded-full font-bold tracking-widest uppercase shadow-lg hover:bg-[#C5A059] transition-colors duration-200 text-xs md:text-sm">ADD TO CART</button>
              )}
            </motion.div>
          </>
        )}
              ...
              <div className="flex-1 overflow-y-auto space-y-4">
                {cart.length === 0 ? <p className="text-gray-400 text-sm">Your bag is empty.</p> : cart.map((item) => (
                  <motion.div layout key={item.cartId} className="flex gap-4 bg-white p-3 rounded-xl shadow-sm relative group items-center">
                    <img src={item.img} className="w-12 h-12 md:w-16 md:h-16 object-cover rounded-lg" alt={item.name} />
            ...
            <motion.div initial={{ x: "100%" }} animate={{ x: 0 }} exit={{ x: "100%" }} transition={{ type: "spring", damping: 25, stiffness: 200 }} className="fixed right-0 top-0 h-full w-full md:w-[450px] bg-white z-[230] p-6 md:p-10 shadow-2xl overflow-y-auto">
              ...
              <form onSubmit={handleFinalCheckout} className="space-y-4">
                ...
                <div className="bg-gray-50 p-4 rounded-lg border border-gray-100 flex items-start gap-3 mt-4">
                  ...
                  <label htmlFor="billAgreement" className="text-[10px] text-gray-600 font-medium cursor-pointer leading-relaxed">I agree to send a picture of my electricity bill on WhatsApp to verify my address.</label>
                </div>
